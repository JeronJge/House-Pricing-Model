...
# THIS DATASET IMPUTES VALUES WHEN JUST 1 OF THE 4 VALUES ARE MISSING

# THE 4 ARE FLOOR SQUARE AREA, BEDROOMS, BATHROOMS AND LIVING ROOMS

# THIS DATASET IS USED FOR THE MODELS BUT NOT FOR INITIAL DATA EXPLORATION
# THIS IS BECAUSE THIS HAS ASSUMPTIONS, OF COURSE, GIVEN THAT WE ARE IMPUTING VALUES
# SO WE CHOSE NOT TO USE IT FOR THE INITIAL PART
# HOWEVER WE ONLY IMPUTED AROUND 12% OF THE VALUES IN THIS DATASET
# THIS LED TO THE RESULTS OF THE REGRESSIONS BEING QUITE SIMILAR
...
out_final  <- "kaggle_cpih_winsorized.csv"
...
dt_ok <- dt[any_missing == FALSE]
dt_ok <- dt_ok[!is.na(history_date_parsed) & !is.na(history_price_num) & history_price_num > 0]
dt_ok[, row_id := .I]
...
dt_latest[, c(
  "living_missing","bath_missing","bed_missing","area_missing",
  "tenure_missing","sale_missing","any_missing","row_id"
) := NULL]
